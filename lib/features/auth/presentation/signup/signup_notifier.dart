...
class SignupNotifier extends Notifier<SignupState> {
  ...
  void setEmail(String v) => state = state.copyWith(email: v, errorMessage: null);
  void setDisplayName(String v) => state = state.copyWith(displayName: v);
  void setPassword(String v) => state = state.copyWith(password: v);
  void setPasswordConfirm(String v) => state = state.copyWith(passwordConfirm: v);
  ...
  Future<void> submit({String? invitationToken}) async {
    ...
    try {
      final resp = await ref.read(authRepositoryProvider).register(
            email: state.email,
