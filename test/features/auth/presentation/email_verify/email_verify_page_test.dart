...
List<GoRoute> _buildRoutes() => [
      ...
      GoRoute(
        ...
        builder: (_, state) => Scaffold(
          body: Text('result:${state.uri.queryParameters['status']}'),
        ),
      ),
    ];
...
void main() {
  group('EmailVerifyPage', () {
    ...
    testWidgets('検証成功: /auth-result?status=successに遷移する', (tester) async {
      final overrides = [
        emailVerifyResultProvider('test-token').overrideWith(
          (ref) async => EmailVerifyResult.success,
        ),
      ];
    ...
    testWidgets('検証期限切れ: /auth-result?status=expiredに遷移する', (tester) async {
      final overrides = [
        emailVerifyResultProvider('test-token').overrideWith(
          (ref) async => EmailVerifyResult.expired,
        ),
      ];
      await tester.pumpWidget(buildTestPageWithRouter(
        routes: _buildRoutes(),
    ...
    testWidgets('検証無効: /auth-result?status=invalidに遷移する', (tester) async {
      final overrides = [
        emailVerifyResultProvider('test-token').overrideWith(
          (ref) async => EmailVerifyResult.invalid,
        ),
      ];
      await tester.pumpWidget(buildTestPageWithRouter(
        routes: _buildRoutes(),
